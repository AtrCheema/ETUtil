...
def process_axes(_axis,
                 ...
                 xtp_ls=20,  # x tick_params labelsize
                 ytp_ls=18,  # x tick_params labelsize
    ...
    if show_leg:
        _axis.legend(loc=leg_pos, fontsize=leg_fs, markerscale=leg_ms)

    if show_ylabel:
        _axis.set_ylabel(y_label, fontsize=yl_fs, color=ylc)

    if log:
        _axis.set_yscale('log')

    if invert_yaxis:
        _axis.set_ylim(_axis.get_ylim()[::-1])
    ...
    _axis.tick_params(axis="x", which='major', labelsize=xtp_ls, colors=xtpc)

    if show_ylabel:
        _axis.tick_params(axis="y", which='major', labelsize=ytp_ls, colors=ytpc)

    _axis.get_xaxis().set_visible(show_xaxis)

    if show_xaxis:
        _axis.set_xlabel(x_label, fontsize=xl_fs)

    _axis.spines['top'].set_visible(top_spine)
    _axis.spines['bottom'].set_visible(bottom_spine)

    if max_xdates is not None:
        _loc = mdates.AutoDateLocator(minticks=max_xdates - 1, maxticks=max_xdates)
        _axis.xaxis.set_major_locator(_loc)
        _fmt = mdates.AutoDateFormatter(_loc)
        _axis.xaxis.set_major_formatter(_fmt)
